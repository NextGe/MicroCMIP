...
namespace EFWCoreLib.CoreFrame.ProcessManage
{
    public class TimeCDKEY
    {
        ...
        //激活注册码
        public static bool ActivateRegCode(string regcode)
        {
            /* 比较CPUid */
            string CpuId = TimeCDKEY.GetSoftEndDateAllCpuId(1, regcode);   //从注册表读取CPUid
            string CpuIdThis = GetCpuId();           //获取本机CPUId         
            if (CpuId != CpuIdThis)
            {
                return false;
            ...
            /* 比较时间 */
            string NowDate = TimeCDKEY.GetNowDate();
            string EndDate = TimeCDKEY.GetSoftEndDateAllCpuId(0, regcode);
            if (Convert.ToInt32(EndDate) - Convert.ToInt32(NowDate) < 0)
            {
                return false;
            }

            WriteSetting(regcode);
            return true;
        }
        /*CPUid*/
        ...
        public static string ReadSetting()
        {
            ...
            if (key1 != "")
            {
                return Encryption.DisEncryPW(key1, "kakake888");
            }
            return "-1";
        }
    }
