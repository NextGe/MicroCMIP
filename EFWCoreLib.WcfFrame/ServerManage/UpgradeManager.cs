...
namespace EFWCoreLib.WcfFrame.ServerManage
{
    ...
    {
        private static string clientupgrade = @"FileStore\ClientUpgrade\";//客户端升级包路径
        private static string webupgrade = @"FileStore\WebUpgrade\";
        private static string mnodeupgrade = @"FileStore\MNodeUpgrade\";
        private static string pluginupgrade = @"FileStore\PluginUpgrade\";
        ...
        {
            //创建客户端升级包目录
            if (!Directory.Exists(AppGlobal.AppRootPath + clientupgrade))
            {
                Directory.CreateDirectory(AppGlobal.AppRootPath + clientupgrade);
            }
            //创建Web程序升级包目录
            if (!Directory.Exists(AppGlobal.AppRootPath + webupgrade))
            {
                Directory.CreateDirectory(AppGlobal.AppRootPath + webupgrade);
            }
            //创建中间件程序升级包目录
            if (!Directory.Exists(AppGlobal.AppRootPath + mnodeupgrade))
            {
                Directory.CreateDirectory(AppGlobal.AppRootPath + mnodeupgrade);
            }
            //创建插件服务程序升级包目录
            if (!Directory.Exists(AppGlobal.AppRootPath + pluginupgrade))
            {
                Directory.CreateDirectory(AppGlobal.AppRootPath + pluginupgrade);
            }
        ...
        public static void DownLoadPlugin(ClientLink _clientLink)
        {
            ...
            if (addplugin.Count > 0 || updateplugin.Count > 0 || deleteplugin.Count > 0)
            {
                ...
                WcfGlobal.normalIPC.CallCmd(IPCName.GetProcessName(IPCType.efwplusServer), "restartbase", null);
            }
        }
        //下载升级包
        public static void DownLoadUpgrade(string updatexml, string updatezip, ClientLink _clientLink)
        {
            ...
            FileInfo finfo = new FileInfo(AppGlobal.AppRootPath + updatexml);
        ...
        private static Version readLocalUpdateXml(string updatexml)
        {
            ...
            xmlDoc.Load(AppGlobal.AppRootPath + updatexml);
        ...
        private static void downloadRemotePlugin(string pluginzip, ClientLink _clientLink)
        {
            ...
            df.FileType = 0;
            FileStream fs = new FileStream(AppGlobal.AppRootPath + pluginzip, FileMode.Create, FileAccess.Write);
