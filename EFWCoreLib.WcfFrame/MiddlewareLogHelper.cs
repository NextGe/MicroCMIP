...
namespace EFWCoreLib.CoreFrame.Common
{
    ...
    public class MiddlewareLogHelper
    {
        ...
        private static void WriterFile()
        {
            foreach (var item in LogNameDic)
            {
                ...
                if (string.IsNullOrEmpty(info) == false)
                {
                    ...
                    if (!System.IO.Directory.Exists(AppGlobal.AppRootPath + "ServerLog\\" + LogNameDic[item.Key]))
                    {
        ...
        public static string ReadFile(string logtype,string date)
        {
            string filepath = AppGlobal.AppRootPath + "ServerLog\\" + logtype + "\\" + date + ".txt";
